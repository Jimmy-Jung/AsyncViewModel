...
struct AsyncEffectTests {
    ...
    func equatable_noneIsAlwaysEqual() {
        #expect(AsyncEffect<MockAction>.none == .none)
    }

    @Test("Equatable: .action은 연관값이 동일할 때만 동일하다")
    func equatable_actionIsEqualWithSameAssociatedValue() {
        #expect(AsyncEffect<MockAction>.action(.increment) == .action(.increment))
        #expect(AsyncEffect<MockAction>.action(.increment) != .action(.decrement))
    }
    ...
    func equatable_runIsEqualWithSameID() {
        ...
        #expect(AsyncEffect.run(id: MockCancelID.taskA, operation: op1) == .run(id: MockCancelID.taskA, operation: op2))
        #expect(AsyncEffect.run(id: MockCancelID.taskA, operation: op1) != .run(id: MockCancelID.taskB, operation: op1))
    }
    ...
    func equatable_runIsEqualWithNilID() {
        ...
        #expect(AsyncEffect.run(operation: op1) == .run(operation: op2))
    }

    @Test("Equatable: .cancel은 ID가 동일할 때 동일하다")
    func equatable_cancelIsEqualWithSameID() {
        #expect(AsyncEffect<MockAction>.cancel(id: MockCancelID.taskA) == .cancel(id: MockCancelID.taskA))
        #expect(AsyncEffect<MockAction>.cancel(id: MockCancelID.taskA) != .cancel(id: MockCancelID.taskB))
    }

    @Test("Equatable: .merge는 내부 배열이 동일할 때 동일하다")
    func equatable_mergeIsEqualWithSameEffects() {
        let effect1: AsyncEffect<MockAction> = .action(.increment)
        let effect2: AsyncEffect<MockAction> = .action(.decrement)
    ...
    func equatable_differentEffectTypesAreNotEqual() {
        #expect(AsyncEffect<MockAction>.none != .action(.increment))
        #expect(AsyncEffect<MockAction>.action(.increment) != .cancel(id: MockCancelID.taskA))
    }
    ...
    func convenience_runActionReturnsActionOnSuccess() async {
        let effect: AsyncEffect<MockAction> = .runAction { .increment }
    ...
    func convenience_runActionReturnsErrorOnFailure() async {
        ...
        let effect = AsyncEffect<MockAction>.runAction { throw TestError.failure }
    ...
    func convenience_runActionsReturnsActionsOnSuccess() async {
        ...
        let effect: AsyncEffect<MockAction> = .runActions { actions }
